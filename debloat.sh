...
KICK() {
    ...
    local APP_DIRS=(
        "$EXTRACTED_FIRM_DIR/system/app"
        "$EXTRACTED_FIRM_DIR/system/priv-app"
        ...
        "$EXTRACTED_FIRM_DIR/system/product/app"
        "$EXTRACTED_FIRM_DIR/system/product/priv-app"
    )

    for app in "${APPS_LIST[@]}"; do
        for dir in "${APP_DIRS[@]}"; do
            if [ -d "$dir" ]; then
                find "$dir" -maxdepth 2 -iname "*${app}*" -exec rm -rf {} + 2>/dev/null \; || true
            fi
        done
    done
}
...
DEBLOAT() {
    ...
    if [ ! -d "$EXTRACTED_FIRM_DIR/system" ]; then
        echo -e "${RED}No extracted firmware found.${NC}"
        return 1
    fi
